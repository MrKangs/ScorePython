...
print("* 다음 질문에 붙어라! 그리고 파일 이름 & extension 까지 적고 enter 하세요.*")
print("*************************************************************************")
location_for_file = str(input("파일이 어디에 위치 되어있을까요?: "))
counter = int(input("데이트의 개수는?: "))
location_for_score = str(input("점수열(column)는 어디?: "))
location_for_name = str(input("학생 이름 명단열(column) 어디?: "))
x_axis = str(input("x축(학생이름) 명은?: "))
y_axis = str(input("y축(점수열) 명은?: "))
min_unit = int(input("그래프의 점수열의 최소 값은?: "))
max_unit = int(input("그래프의 점수열의 최대 값은?: "))
units = int(input("그래프 상 점수열의 단위 크기는(예 1, 5 또는 10 등)? "))
Title = str(input("그래프 제목명(title)은?: "))
